    ...
    async def _generate_report_service(self, call: ServiceCall) -> None:
        """Service to generate comprehensive report."""
        try:
            self._update_stats("maintenance_actions")
            
            report_type = call.data.get("report_type", "daily")
            include_charts ="""Script platform for Hundesystem integration - SMART ACTION SCRIPTS."""
...
async def async_setup_entry(
    ...
    """Set up Hundesystem scripts and services."""
    dog_name = config_entry.data[CONF_DOG_NAME]
    
    # Create script manager
    script_manager = HundesystemScriptManager(hass, config_entry, dog_name)
    
    # Register all services
    await script_manager.async_setup_services()
    
    _LOGGER.info("Successfully set up Hundesystem scripts for %s", dog_name)


class HundesystemScriptManager:
    """Manager for Hundesystem scripts and services."""
